...
class TestEpsilonGreedyBandit(TestBandit):
    ...
    def test__choose_explores_with_low_rng(
        ...
        rng = mocker.MagicMock(random=lambda: 0.9 * valid_params["eps"])
        primed_bandit._choose(rng)
        rng.integers.assert_called_once_with(0, prime_params["k"])

    def test__choose_exploits_with_high_rng(
        self, mocker, valid_params, primed_bandit, Qs
    ):
        rng = mocker.MagicMock(random=lambda: 1.1 * valid_params["eps"])
        primed_bandit._Qs = Qs
        choice = primed_bandit._choose(rng)
